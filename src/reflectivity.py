...
This module moved here from ``eigsep_terrain.reflectivity``: it is
simulation physics rather than terrain geometry, and both the ground-based
and lunar forward models need it.
...
import warnings
import numpy as np
from eigsep_base.const import c as _C

# NOTE: the float32 downcast is inherited verbatim from the original
# eigsep_terrain.reflectivity so that eps_r_from_gpr keeps returning bit
# identical results. It costs ~1e-7 relative accuracy in the speed of
# light, far below GPR travel-time precision, but it is almost certainly
# unintentional -- drop the cast if you ever revisit this.
C = np.float32(_C)
